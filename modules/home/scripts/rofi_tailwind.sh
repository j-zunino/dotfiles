...
ordered_names=(slate gray zinc neutral stone red orange amber yellow lime green emerald teal cyan sky blue indigo violet purple fuchsia pink rose)
ordered_scales=(50 100 200 300 400 500 600 700 800 900 950)

declare -A pretty_to_raw

rofi_input=""
for name in "${ordered_names[@]}"; do
    for scale in "${ordered_scales[@]}"; do
        key="${name}-${scale}"
        hex="${tailwind_colors[$key]}"
        if [[ -n $hex ]]; then
            pretty="<span foreground=\"$hex\">██</span> <span>$key</span>: $hex"
            pretty_to_raw["$pretty"]="$key:$hex"
            rofi_input+="$pretty"$'\n'
        fi
    done
done

selected_pretty=$(echo "$rofi_input" | rofi -dmenu -l 11 -markup-rows -p "  ")
[[ -z $selected_pretty ]] && exit 0

raw="${pretty_to_raw[$selected_pretty]}"
name="${raw%%:*}"
hex="${raw##*:}"

copy_choice=$(printf "%s\n%s" "$name" "$hex" | rofi -dmenu -p "Copy  ")
[[ -z $copy_choice ]] && exit 0

if [[ $copy_choice == "$name" ]]; then
    to_copy="$name"
elif [[ $copy_choice == "$hex" ]]; then
    to_copy="$hex"
else
    exit 1
fi
...
if command -v wl-copy &>/dev/null; then
    echo -n "$to_copy" | wl-copy
else
    notify-send "Error" "No clipboard tool found (need wl-copy or xclip)"
    exit 1
fi

notify-send "Copied to clipboard" "$to_copy"
